...
class IlluminationMap:
    ...
    def _avg_impurity_from_map(self, coverage_data, misbehavior_data):
        # If there are no samples, we cannot compute it
        if np.count_nonzero(coverage_data) == 0:
            return np.NaN  

        # https://numpy.org/doc/stable/reference/arrays.nditer.html
        # This should iterate over all the elements
        it = np.nditer(coverage_data, flags=['multi_index'])
        samples = []
        for a in it:
        ...
        for sample in samples:
            correct = coverage_data[sample] - misbehavior_data[sample]

            # Increment number of observations
            k += 1

            # compute impurity
            if misbehavior_data[sample] == 0 or correct == 0:
                impurity = 0
            else:
                impurity = 1 - (misbehavior_data[sample]/coverage_data[sample])**2 + \
                (correct/coverage_data[sample])**2
            map_impurities[sample] = impurity

        # Compute the average
        if len(map_impurities) < 1:
            return 0.0
        else:
            return np.mean([list(map_impurities.values())])
    ...
    def compute_statistics(self, tags=[], feature_selector=None, sample_selector=None):
        ...
        for feature1, feature2 in itertools.combinations(filtered_features, 2):
            ...
            map_report = {
                ...
                'AUC Filled Cells': self._auc_filled_cells(coverage_auc_data, time_data),
                'AUC Mapped Misbehaviors': self._auc_mapped_misbehaviors(misbehavior_auc_data, time_data),
                'Avg Impurity': self._avg_impurity_from_map(coverage_data, misbehavior_data)
            }
